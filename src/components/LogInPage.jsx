...
//                 <div onClick={() => setShowPassword(!showPassword)} className="cursor-pointer mb-3">
//                     <svg className='absolute right-0  mr-4 mb-1' width="24" height="24" fill="none" viewBox="0 0 24 24">
...
const LoginPage = () => {
    ...
    const onSubmit = (data) => {
        const allUsers = JSON.parse(localStorage.getItem('sakroobUsers')) || [];
        const user = allUsers.find(u => u.email === data.email.toLowerCase() && u.password === data.password);
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100">
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-8 rounded shadow-md max-w-md w-full">
                ...
                <input {...register("password", { required: "Password is required" })} placeholder="Password" type="password" className="w-full mb-2 p-2 border rounded" />
                {errors.password && <p className="text-red-500 text-sm">{errors.password.message}</p>}

                <button type="submit" className="w-full bg-blue-600 text-white p-2 rounded">Login</button>

                <p className="text-center mt-4">Don't have an account? <NavLink to="/signup" className="text-blue-600">Sign Up</NavLink></p>
            </form>
            <ToastContainer />
        </div>
    );
